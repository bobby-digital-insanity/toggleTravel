...
// Identify the selected user in Datadog RUM
function ddRumApplyUser(user) {
  if (!window.DD_RUM) return;
  if (user) {
    window.DD_RUM.setUser({ id: user.email, email: user.email, name: user.name, tier: user.key });
  } else if (typeof window.DD_RUM.removeUser === 'function') {
    window.DD_RUM.removeUser();
  }
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // User select
  injectUserSelect();
  ddRumApplyUser(getCurrentUser());

  // Re-identify on tier change
