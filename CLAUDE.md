...
- **LD agent graph `ai-planner-graph`** — the roster and wiring are LD config, not code. Root = `ai-planner-agent` (PLAN only); one edge per specialist; each specialist has its own edge into `ai-planner-agent-compose` (COMPOSE), the graph's sole terminal node. Root → 4 specialists → compose is a real DAG — PLAN and COMPOSE are two separate AI Configs, not two calls against the same node, so the LD graph UI draws two distinct boxes with two distinct Monitoring histories instead of one node appearing twice via a cycle. Each root→specialist edge's **handoff JSON** carries that specialist's `label`, `timeoutMs`, `pass` and `reportFormat`, which the prompts and the per-specialist timeout read at runtime. Adding a fifth specialist or retuning a timeout is a graph edit in LD with no deploy. Backed by a plain multivariate flag whose `default` variation is `{root, edges}`; REST is `/api/v2/projects/ToggleTravel/agent-graphs/ai-planner-graph` (`PATCH` needs `rootConfigKey` **and** `edges` together, and LD assigns its own edge keys). `COMPOSE_KEY` in `agents/supervisor.js` falls back to the root if the compose node is ever missing from the graph, at the cost of the duplicate-box problem returning.
...
- **LD AI Configs (agent mode)** — `ai-planner-agent` (PLAN; two variations with a targeting rule on the `complexity` context attribute: Sonnet 5 for `simple`, Opus 5 for `complex`) and `ai-planner-agent-compose` (COMPOSE; the same targeting rule, mirrored — kept in sync by hand, not derived), plus specialists `ai-planner-agent-weather` (Haiku 4.5), `-location` (Sonnet 5), `-budget` (Haiku 4.5), `-timing` (Sonnet 5). Model choice is entirely LD config.
...
- **Two judges, two failure modes.** `ai-planner-judge` (accuracy) catches fabricated *data* — a guessed fare, an invented flight status — and is the only judge that triggers a retry. `relevance` catches the assistant answering the wrong thing entirely: off-topic drift, or complying with a prompt injection embedded in the user's message. Both run in `routes/aiPlanner.js` via `runJudgeConfig()`, one function behind `judgeAnswer()` and `judgeRelevance()`; relevance is report-only in both modes — a scope failure isn't fixed by asking a stronger model the same question. Thresholds are the LD number flags `ai-planner-judge-threshold` / `ai-planner-relevance-threshold` (default 0.7). The `relevance` and `toxicity` LD configs also ship attached to the `ai-planner` completion config's `judgeConfiguration`, but that attachment is never invoked — this route calls provider `chat()` functions directly rather than `ManagedModel.run()`, which is the only thing that consults it — so `toxicity` is still config-only/dead; only `relevance` is wired into code, explicitly. The `relevance` LD config's rubric expects `{{message_history}}`/`{{response_to_evaluate}}` (LD's reserved judge template names) rather than `ai-planner-judge`'s `{{message}}`/`{{response}}`; `runJudgeConfig()` supplies both pairs so either convention resolves.
