 ...
 *
 * Sentry-branch notes:
 *   - No experiment/MAB setup runs here (that is the launchdarkly branch). This
 *     branch ships feature flags + the guarded-rollout incident only.
 *   - The traffic this generates is what fills Sentry: server transactions from
 *     the API tier, browser sessions + replays from the browser tier, and a
 *     daily burst of flag-tagged 500s from the incident.
 *   - LD_PROJECT_KEY / LD_ENV_KEY default to the Sentry project's values; verify
 *     them or the incident silently no-ops.
 *
 * Env knobs (defaults):
 ...
 *   LD_API_TOKEN / LD_PROJECT_KEY / LD_ENV_KEY / LD_API_BASE
 */
